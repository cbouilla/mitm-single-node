...
static void router_usage()
{
	...
	printf("  --seconds X --no-bind --cache-level L --local-only     (bench)\n");
	printf("  --test NAME                      (test) connect_only basic tiny rounds zero skew partial_lines\n");
	printf("                                   pop_sizes swc_is_block staggered_close slow_recv asymmetric all\n");
	printf("  --quiet\n");
	exit(EXIT_SUCCESS);
}
...
static void router_parse(int argc, char **argv, RouterArgs &a)
{
	enum {
		...
		O_SEED, O_SECONDS, O_NOBIND, O_CACHE, O_LOCAL, O_TEST, O_QUIET, O_HELP
	};
	...
	for (;;) {
		...
		case O_NOBIND: a.bind = false; break;
		case O_CACHE: a.cache_level = atoi(optarg); break;
