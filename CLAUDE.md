...
Topology (the `Parameters` constructor in `include/parameters.hpp`): **one MPI rank per node**, `MPI_THREAD_FUNNELED`. Inside a rank, thread 0 is the comm thread (and, on rank 0, the controller), threads `1..inserters_per_node` own a dictionary shard each, the rest walk trails. Walkers default to filling the inherited affinity mask, hence `--bind-to none`. **The engine assumes one MPI rank per NUMA node** and rank 0 warns loudly otherwise. Placement is cache- and SMT-aware (hwloc): the mask's **cores** are cut into **one thread group per inserter**, each inside one cache domain — the lowest level shared by several *cores*, L3 on both Xeon and EPYC, detected rather than hardcoded and overridable with `--cache-level`. Groups own whole cores so that a core's SMT siblings stay in one group. Every thread then takes the **emptiest core of its group**, service threads first, which gives the comm thread and each inserter a core of its own and hands the siblings they leave to walkers — a walker fills the issue slots a thread waiting on queues or DRAM leaves idle, and two waiting threads must never share a core (`PROBLEM.md` §3: ~10% to them, +66% to the walkers). **A walker's group IS the inserter whose collisions it resolves**, so `--inserters-per-node` sets the group count: make it at least the number of cache domains (rank 0 warns otherwise) and never more than `--walkers-per-node` (refused: every collision queue needs a walker). `--no-bind` disables pinning and falls back to `walker_index % inserters_per_node`. Each thread records the CPU and NUMA node the kernel reports after pinning in its `ThreadContext` (`cpu`, `numa_node`); a thread that isn't where it should be aborts the run before anything is allocated. Other topologies are testable on a laptop with `HWLOC_SYNTHETIC="node:1 l3:4 core:4 pu:1"` set on the *application* (`mpirun ... env HWLOC_SYNTHETIC=... build/examples/...`), not on `mpirun`, whose own mapper would then see the fake machine. `--nrounds` bounds `max_versions` so a search can report "not found" instead of running forever. On a CEA/TGCC-style cluster the launcher is `ccc_mprun` under an `MSUB` batch script (`tgcc.sh`).
