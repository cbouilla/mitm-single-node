...
**There is one engine core (MPI + OpenMP), templated on a *scheme*.** The core — `engine.hpp`, `comm.hpp`, `controller.hpp`, `parameters.hpp`, `spsc.hpp`, `placement.hpp` — moves points from producer threads to dictionary shards and runs the rounds; a scheme (`PROTOCOL.md` §7) says what a point is, what the dict threads and the producers do with it, when a round ends and how it is reported. Today's only scheme is PCS (van Oorschot–Wiener parallel collision search over a distributed dictionary): `pcs_common.hpp`, `walker.hpp`, `inserter.hpp`, `pcs.hpp`, entry points `mitm::pcs::claw_search(pb, nbytes_memory, opts, prng)` and `mitm::pcs::collision_search(...)`. The direct (exhaustive) scheme is being added as `mitm::direct::` with the same signatures. The sequential and "vector sequential" engines were deleted; a single rank with one producer and one dict thread is the degenerate sequential case. Do not reintroduce an engine-selection template parameter on the entry points: a driver picks a scheme by namespace (`--engine` in the demos), and the entry points take no engine.
