...
- `counters.hpp` — `enum counter` and `Counters`: every diagnostic tally of a thread in one plain `u64[N_COUNTERS]` (no atomics: the comm thread sums them after an `omp flush` for the progress reports, and reads them exactly after the end-of-round barrier), plus the HyperLogLog. The same array is the wire layout of a progress report (deltas) and of the end-of-round `MPI_Reduce` (totals). **Round-scoped only**: threads accumulate, the comm thread merges and reduces them, and all-time totals plus every `printf` live in the controller.
...
- `walker.hpp` / `inserter.hpp` — the two worker threads; `walker.hpp` also holds the trail code (`walk`, `walk_nolen1`, `resolve_collision`). `dict.hpp` — `PcsDict`, the dictionary shard an inserter owns. `comm.hpp` / `spsc.hpp` — queues, bulk DP buffers, control channel, per-thread state (`ThreadContext` owns the worker's queue and, for an inserter, its shard). `controller.hpp` — rank 0's rounds and statistics. `engine.hpp` — `CommThread` (thread 0, all of it: routing, control channel, drain, epilogue, termination) and `run()`, the engine's one entry point: preflight, the OpenMP team and its round loop; what the threads share (`Parameters`, the `ctx` table, the collision queue, the round state) are its locals.
...
- **Per-thread objects are built by their owning thread**, in `run()` right after `pin_to_cpu`: its `ThreadContext`, whose constructor builds what the thread owns, the SPSC queue and, for inserters, the `PcsDict` shard; thread 0 builds its `CommThread` (the MPI buffers and channels) once the slots are published. Before the team exists, `run()` only derives the `Parameters` and sizes the `ctx` table. This is NUMA first touch — the shard's zero-fill is what places its pages — so don't move construction back into the constructor. Deliberately left for later: a NUMA-affine parallel flush of the shards between rounds (needs topology the engine doesn't have), and a generation bit in dictionary entries to skip the flush altogether (changes the "empty at round start" guarantee in `PROTOCOL.md`).
