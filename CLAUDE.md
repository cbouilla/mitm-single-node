...
- No CTest/unit-test suite. The demos self-check with `assert()` against a planted golden pair; running `double_speck64_demo` on a small `--n` is the smoke test, with both `--engine`s.
